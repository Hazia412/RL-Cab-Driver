...
class CabDriver:
    def __init__(self):
        ...
        self.action_space = list(permutations([i for i in range(m)], 2)) + [
            (0, 0)
        ]  ## All permutaions of the actions and no action
        self.state_space = [
            [x, y, z] for x in range(m) for y in range(t) for z in range(d)
        ]  ##
        # Based on s=𝑋𝑖𝑇𝑗𝐷k
    ...
    def next_state_func(self, state, action, Time_matrix):
        ...
        # Finding current state of driver
        ...
        # Reward depends of time, lets initialize to 0
        total_time = 0
        wait_time = 0
        ride_time = 0
        transit_time = 0

        # Next state depends on possible actions taken by driver
        # There are 3 cases in this scenario:
        # 1. Driver refuses i.e. action (0,0)
        if (pickup_location) == 0 and (drop_location == 0):
            wait_time = 1
            next_loc = current_location

        # 2. Driver wants to pickup and is at same location
        elif pickup_location == current_location:
            ride_time = Time_matrix[current_location][drop_location][current_time][
                current_day
            ]
        ...
        # 3. Driver wants to pickup and is at different location
            ...
            transit_time = Time_matrix[current_location][pickup_location][current_time][
                current_day
            ]
            updated_time, updated_day = self.get_updated_day_and_time(
                current_time, current_day, transit_time
            )
            ride_time = Time_matrix[pickup_location][drop_location][updated_time][
                updated_day
            ]
            next_loc = drop_location
